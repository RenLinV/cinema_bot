...
headers = {"X-API-KEY": "ATPR3RR-A3YM7EQ-MHJE5RK-1M1YN3N"}

bot = Bot(token="6409418300:AAGEZvcMmGQ12G8sW1ubBP9CIuCHauXkHTs")
...
async def find_by_name(message: types.Message, state: FSMContext):
    movie_info, url, t = film_info.get_by_name(message, headers)
    await bot.send_photo(message.chat.id, t[:4080])
    #await bot.send_photo(message.chat.id, photo=url, caption=t)
